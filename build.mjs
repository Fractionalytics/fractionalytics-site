...
/* ------------------------------------------------- heading width cap check */

// A CSS max-width chosen by feel becomes a silent editorial constraint: it decides how long a
// heading may be, nobody declares it, and it stays invisible until someone writes one that trips
// it. That happened twice here. h2 was capped at 30ch against a 68ch prose measure, holding every
// heading to half the body width; the hand-measured replacement, 40ch, was measured against the
// one page that was open and shipped with a comment claiming it cleared everything. It did not.
// Three article headings exceed it, and the fix reproduced the defect inside one session.
//
// THE CHECK IS IN ch, NEVER px. Both the text width and the ch unit scale with font-size, so a
// ch-space comparison is viewport-independent and the clamp() on these rules stops mattering. No
// browser needed.
//
// THE CONSTANT IS MEASURED, AND THE FIRST VALUE TRIED WAS WRONG. 0.765 was reported from one
// sampling and under-predicted every real heading by about 12%, which would let a 45ch heading
// pass a 40ch cap. Re-derived against five headings rendered live on the deployed site: 0.844,
// 0.857, 0.877, 0.858 ch per character for the four over 45 characters, so 0.86 with a max
// deviation of 2.1%. The fifth, a 25-character heading, sits at 0.924 and is ignored on purpose:
// short headings never approach a cap, and including them drags the constant away from the range
// where it has to be right. Re-measure if a display face changes.
//
// THE intent FLAG IS THE POINT, not the number. A cap either DERIVES from the longest content that
// exists, in which case content exceeding it is a defect worth warning about, or it BINDS by
// intent, in which case wrapping is the design and a warning is noise. h1 at 22ch and .lede at
// 56ch are both far narrower than their longest content and both are correct, because display type
// is meant to stack. Without this flag the check fires on every cap, is wrong about most of them,
// and gets switched off within a week.
const CH_PER_CHAR = { Oswald: 0.86, Archivo: 0.81 };
const MEASURE_CH = 68;

const HEADING_CAPS = [
  { tag: 'h2', capCh: 40, face: 'Oswald', intent: 'binding',
    note: 'long article titles wrap to two lines on purpose' },
  { tag: 'h1', capCh: 22, face: 'Oswald', intent: 'binding',
    note: 'display headlines are meant to stack' },
];

let capWarnings = 0;

function checkHeadingCaps(rel, html) {
  const out = [];
  for (const c of HEADING_CAPS) {
    if (c.intent === 'binding') continue;
    const re = new RegExp('<' + c.tag + '[^>]*>([^]*?)</' + c.tag + '>', 'g');
    for (const m of html.matchAll(re)) {
      // .post-list h2 overrides max-width to none, so the cap does not bind there and checking
      // it produced a false positive on the writing index. Those headings are the only ones that
      // wrap a link, which is a precise test; a look-behind window is not, because later list
      // items sit further from the opening <ol> than any fixed window.
      if (/<a[\s>]/.test(m[1])) continue;
      const text = m[1].replace(/<[^>]+>/g, '').replace(/&[a-z]+;/g, ' ').trim();
      const wantCh = text.length * CH_PER_CHAR[c.face];
      // 5% tolerance covers the width-model error, which measured under 3%.
      if (wantCh > c.capCh * 1.05) {
        out.push('  ! ' + rel + ': <' + c.tag + '> wants ~' + wantCh.toFixed(0)
          + 'ch but is capped at ' + c.capCh + 'ch (measure is ' + MEASURE_CH
          + 'ch), so it wraps with ' + (MEASURE_CH - c.capCh)
          + 'ch of measure empty beside it: "' + text.slice(0, 46) + '"');
      }
    }
  }
  return out;
...
function write(rel, contents) {
  ...
  if (rel.endsWith('.html')) {
    for (const w of checkHeadingCaps(rel, contents)) { console.log(w); capWarnings++; }
  }
}
...
- [The practice](${SITE}/): the wall and three anonymized cases with real numbers, then the three engagements. Remediation, when something is already broken, in three parts. Diligence, before you own it. Fractional CDO, when it is not one thing. Then the three shapes the work can be bought in, which systems it touches, and why the economics moved.
...
if (capWarnings) {
  console.log(`\n${capWarnings} heading(s) exceed a DERIVED width cap. Either raise the cap in`);
  console.log(`styles.css and re-measure against EVERY page, or mark that cap intent: 'binding'`);
  console.log(`and say in the CSS comment what it wraps to. Do not widen it against the one page`);
  console.log(`you happen to have open, which is how the 40ch replacement got written wrong.`);
}
